...
def render_set(model_path, name, iteration, views, gaussians, pipeline, background, render_normals):
    render_path = os.path.join(model_path, name, "3dgs_dr_{}".format(iteration), "renders")
    gts_path = os.path.join(model_path, name, "3dgs_dr_{}".format(iteration), "gt")

    makedirs(render_path, exist_ok=True)
    makedirs(gts_path, exist_ok=True)

    if render_normals:
        normals_path = os.path.join(model_path, name, "3dgs_dr_{}".format(iteration), "normals")
